function init() {
    ...
    const partPositions = {
        assembled: {
            ...
            lid: new THREE.Vector3(0, 2165, 0)
        },
        disassembled: {
    ...
    const partInfo = {
        corpus: {
            ...
            description: "Основная несущая конструкция реактора, выполненная из ферритно-мартенситные стали.",
            specs: [
                "Материал: ЭП-823",
                "Высота: 2545 мм",
    ...
    function generateTvsPositions(count, spacing, rotationDegrees = 0) {
        ...
        if (count === 7) {
            ...
            // 6  вокруг центральной, смещенные для касания сторонами
            
            // Для касания сторонами (а не углами), 6 внешних  нужно сместить ближе к центру
            // Если центральная  имеет диаметр D, то для касания сторонами расстояние между центрами = D
            ...
            // Угол между  в шестиугольнике (60 градусов)
    ...
    Promise.all([
        ...
        console.log(`✅ Создано ${tvsModels.length} `);
        ...
        console.log('📐 Расстояние между  в сборке:', TVS_HEX_GRID.SPACING_SMALL);
        console.log('📐 Расстояние между  в разборке:', TVS_HEX_GRID.SPACING_LARGE);
    ...
    function disassembleTVS() {
        console.log('🔧 Разъезд 7 ...');
        tvsDisassembled = true;
        
        const animations = tvsModels.map((tvs, index) => {
            // Для центральной  (индекс 0) оставляем на месте
