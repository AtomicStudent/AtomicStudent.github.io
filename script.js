function init() {
    ...
    const MANUAL_POSITION_CORRECTION = {
        ...
        TVS: { x: 0, y: 0, z: 0 },        // Корректировка позиции ТВС
    ...
    const modelPaths = {
        ...
        tvs: 'models/reactor_tvs.glb',            // ТВС отдельно
        ...
        return new Promise((resolve, reject) => {
            loader.load(
                ...
                (gltf) => {
                    ...
                    if (key !== 'assembly') {
                        const correction = MANUAL_POSITION_CORRECTION[key.toUpperCase()];
                        if (correction) {
    ...
    Promise.all([
        ...
        // Отдельные детали пока не добавляем, они будут появляться при разборке
        // Но нам нужно их позиционировать правильно
        setupModelsPosition();
    ...
    function setupModelsPosition() {
        ...
        // Для простоты ставим все детали в центр сцены
        // В реальном проекте здесь нужно точно позиционировать каждую деталь
        
        ['corpus', 'tvs', 'lid'].forEach(key => {
            if (models[key]) {
                ...
                partPositions.disassembled[key] = new THREE.Vector3(
                    ...
                    partPositions.assembled[key].y + 
                        (key === 'corpus' ? DISASSEMBLY_DISTANCE.CORPUS_DOWN : 
                         key === 'lid' ? DISASSEMBLY_DISTANCE.LID_UP : 
                         DISASSEMBLY_DISTANCE.TVS_STAYS),
        ...
        fadeOutModel(models.assembly, ANIMATION_DURATION.FADE, () => {
            ...
            Promise.all([
                ...
                Promise.all([
                    ...
                    console.log('✅ Разборка завершена');
                });
            });
        });
    }
    ...
    function assembleReactor() {
        ...
        // Этап 1: Возвращаем детали в исходные позиции
        Promise.all([
            ...
            Promise.all([
                ...
                fadeOutModel(models.tvs, ANIMATION_DURATION.FADE),
                fadeOutModel(models.lid, ANIMATION_DURATION.FADE)
            ]).then(() => {
                // Убираем детали из сцены
                ['corpus', 'tvs', 'lid'].forEach(key => {
                    scene.remove(models[key]);
                });
    ...
    // ---------- 11. ВЫБОР И ИНФОРМАЦИЯ О ДЕТАЛЯХ ----------
    ...
    // ---------- 12. АНИМАЦИЯ И ОБРАБОТКА РЕСАЙЗА ----------
